...
;(function(exports, global) {
    ...
    var modules =
        (global.define && global.define.modules) || (global._define && global._define.modules) || {}
...
define('syn/synthetic', function(require, exports, module) {
    ...
    extend(syn, {
        ...
        isFocusable: function(elem) {
            ...
            return (
                this.focusable.test(elem.nodeName) ||
                (attributeNode && attributeNode.specified && syn.isVisible(elem))
            )
        },
        isVisible: function(elem) {
            return (
                (elem.offsetWidth && elem.offsetHeight) || (elem.clientWidth && elem.clientHeight)
            )
        },
        tabIndex: function(elem) {
            ...
            return (
                ...
                (parseInt(elem.getAttribute('tabIndex')) || 0)
            )
        },
        bind: bind,
        unbind: unbind,
        ...
        dispatch: function(event, element, type, autoPrevent) {
            if (element.dispatchEvent && event) {
                ...
                return (
                    element.sourceIndex <= 0 ||
                    (element.fireEvent && element.fireEvent('on' + type, event))
                )
            }
        },
        create: {
...
define('syn/key', function(require, exports, module) {
    ...
    h.extend(syn.key, {
        ...
        defaults: {
            ...
            v: function(options, scope, key, force, sel) {
                if (syn.key.ctrlKey) {
                    syn.key.defaults.character.call(
                        this,
                        options,
                        scope,
                        syn.key.clipboard,
                ...
                if (syn.typeable.test(this)) {
                    if (syn.key.shiftKey) {
                        syn.selectText(
                            this,
                            sel.start,
                            sel.end + 1 > getText(this).length ? getText(this).length : sel.end + 1
                        )
                    } else {
                        syn.selectText(
                            this,
    ...
    h.extend(syn.init.prototype, {
        _key: function(element, options, callback) {
            ...
            if (runDefaults) {
                if (!keypressOptions) {
                    defaultResult = getDefault(key).call(
                        element,
                        keypressOptions,
                        h.getWindow(element),
                        key,
...
define('syn/drag', function(require, exports, module) {
    ...
    var elementFromPoint = function(point, element) {
            ...
            el = win.document.elementFromPoint
